...
inline xc_status_t xc_pwmmodlmag_plbw_create(xc_iface_t **iface, void *config_table)
{
    // set up iface
    *iface = (xc_iface_t *) config_table;

#ifdef XC_DEBUG
    PWMMODLMAG_PLBW_Config *_config_table = config_table;
...
inline xc_status_t xc_pwmmodlmag_plbw_release(xc_iface_t **iface) 
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_pwmmodlmag_plbw_open(xc_iface_t *iface)
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_pwmmodlmag_plbw_close(xc_iface_t *iface)
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_pwmmodlmag_plbw_read(xc_iface_t *iface, xc_r_addr_t addr, uint32_t *value)
{
    *value = Xil_In32((uint32_t *) addr);
    return XC_SUCCESS;
}

inline xc_status_t xc_pwmmodlmag_plbw_write(xc_iface_t *iface, xc_w_addr_t addr, const uint32_t value)
{
...
{
    PWMMODLMAG_PLBW_Config *_config_table = (PWMMODLMAG_PLBW_Config *) iface;

    if (strcmp("Config", name) == 0) {
        *shmem = (void *) & _config_table->Config;
    } else if (strcmp("Duty", name) == 0) {
        *shmem = (void *) & _config_table->Duty;
    }
